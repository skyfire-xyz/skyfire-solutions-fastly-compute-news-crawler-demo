...
async function verifySkyfirePayIdHeader(skyfireToken) {
  ...
  try {
    ...
    const data = await redis.hset(skyfireToken, {"skyfireEmail": jwtPayload?.bid?.skyfireEmail});
    return {
      isValid: true,
    };
  } catch (err) {
...
async function handleRequest(event) {
  ...
  if (!skyfirePayIdVerificationRes.isValid) {
    ...
    const redisData = await redis.hset(skyfireToken, {"amountCharged": amountCharged, "remainingBalance": remainingBalance}); //
    console.log("redis data", redisData);

    const data = await redis.hgetall(skyfireToken);
